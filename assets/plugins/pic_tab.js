...
$.fn.banqh = function(can){
	...
	$("#ban_num1 li").width(picw/4-6);
    $("#ban_num1").height($('#ban_num1 li').height()+12);
    $('#demo1').height(pich+$("#ban_num1").height());
	$(can.pic).find('ul').width(picnum*picw);
	$(can.pnum).find('ul').width(picminnum*picminw + 2);
	$(can.pop_pic).find('ul').width(picpopnum*poppicw);
	
	$(can.pic).find('ul li').width(picw);
	$("#ban_pic1").height(pich);
	

	console.log(picminnum+'*'+picw+'----'+picminw+'====='+picminnum*picminw + 2);
//点击小图切换大图
...
//大图切换过程
		function show(tpqhnum){
			...
			$('#text').find('li').addClass('hide').eq(tpqhnum).removeClass('hide');
		};
